...
func (c *Component) validateSearchResponse(actualResponse, expectedResponse transformer.SearchResponse) error {
	...
	//for i, _ := range actualResponse.Items {
		//validateItems(actualResponse.Items[i], expectedResponse.Items[i])
	//}
	//
	return nil
}

func (c *Component) validateItems(actualItem, expectedItem string) {
	assert.Equal(&c.ErrorFeature, actualItem, expectedItem)
}
